(function() {
    ...
    function renderAdminControl(state) {
        ...
        if (state.admin && currentSlug) {
            ...
            deleteButton.addEventListener('click', function() {
                var title = shareTitle || currentSlug;
                var ok = window.confirm('이 글을 삭제할까요?\n\n' + title);
                if (!ok) return;

                deleteButton.disabled = true;
                deleteButton.textContent = '요청 중';
                setAdminMessage(control, '', false);

                apiJson('/api/posts/' + encodeURIComponent(currentSlug) + '/delete', {
                    method: 'POST'
                }).then(function() {
                    deleteButton.textContent = '삭제 대기';
                    setAdminMessage(control, 'GitHub Actions에서 삭제와 배포를 진행합니다.', false);
                }).catch(function(error) {
                    deleteButton.disabled = false;
                    deleteButton.textContent = '삭제';
                    setAdminMessage(control, error.message || '삭제 요청 실패', true);
                });
            });
        ...
        logout.addEventListener('click', function() {
            ...
            apiJson('/api/auth/logout', { method: 'POST' }).then(function() {
                renderAdminControl({ configured: true, authenticated: false, admin: false, user: null });
            }).catch(function() {
    ...
    function initAdminControl() {
        ...
        apiJson('/api/auth/me').then(renderAdminControl).catch(function() {});
    }
